...
| **L1 全系统无头仿真** | ✅ | — | `npm run sim:full` | Node 里运行真实 EntityManager + Rapier + 系统组件，用 BotCore 打穿完整 gta 任务链 |
...
| L1a 规则抽象模拟 | `node tools/simulate.mjs` | 毫秒 | 规则与内容逻辑错误（被捕传送到哪、任务链能否通关） |
| L1b 全系统无头仿真 | `npm run sim:full` | 秒级 | 真实 ECS/物理/系统组件是否能打穿 gta 完整任务链 |
