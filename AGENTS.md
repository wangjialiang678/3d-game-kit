...
16. **装饰物碰撞要过机器人路线审查**：试玩机器人在路点间走直线、无避障，出生点↔任务点↔车位之间的开阔带是它的高速路——散落的带碰撞装饰（停放车辆等）放上去就是可靠性地雷（真实案例：广场停车把车流擦挂进楼里卡死）。装饰车辆一律不带 collider；确需碰撞的大型地标（喷泉）放好后必须双端回归（sim:full + playtest）。
