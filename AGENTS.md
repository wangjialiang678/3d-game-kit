...
12. **玩家反馈用飞行记录仪**：游戏常驻黑匣子（2min 状态环+事件流），卡死/穿模看门狗自动报警，玩家按 **F9** 下载诊断包 JSON 发给老师/AI（`window.__flight.dump()` 亦可）。
13. **每个目标必须有全屏可见引导**（50m 高光柱 + HUD 方向箭头/距离）。"玩家找不到该去哪"按 bug 对待，不是玩家的错。
